'use client'
export default function Header({ onSettingsClick, hasApiKey, username, onLogout }: { onSettingsClick: () => void; hasApiKey: boolean; username?: string; onLogout?: () => void }) {
  return (
    <header className="bg-white/80 backdrop-blur-sm border-b border-gray-200 px-6 py-4">
      <div className="max-w-[1280px] mx-auto flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {username && (
            <span className="text-sm text-gray-500 px-3 py-1.5 bg-gray-50 rounded-lg">{username}</span>
          )}
          {username && onLogout && (
            <button onClick={onLogout} className="text-xs text-gray-400 hover:text-red-500 px-2 py-1.5 rounded-lg hover:bg-red-50 transition-colors cursor-pointer">退出</button>
          )}
